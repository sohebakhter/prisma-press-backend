...
const getSubscriptionStatus = async (userId: string) => {
    const isSubscriptionExists = await prisma.subscription.findFirstOrThrow({
        where: {
            userId
        }
    })

    const isActive = isSubscriptionExists.status === SubscriptionStatus.ACTIVE && isSubscriptionExists.currentPeriodEnd > new Date()
